...
export default function LogInPage() {
  const router = useRouter();
  const [selectedRole, setSelectedRole] = useState("Reader"); // Reader, Writer, Admin
  const [showPassword, setShowPassword] = useState(false);
  ...
  const onSubmit = async (e) => {
    ...
    // Admin এর জন্য লোকাল চেক (আপনার রিকোয়ারমেন্ট অনুযায়ী)
    if (selectedRole === "Admin") {
      if (email === "admin@fable.com" && password === "Admin@123") {
        toast.success("Admin Login Successful!");
        // Admin ড্যাশবোর্ডে পাঠাচ্ছি
        router.push("/dashboard/admin");
        router.refresh();
        return;
      } else {
    ...
    // সাধারণ ইউজার এবং রাইটার এর জন্য Better Auth signIn
    const { data, error } = await authClient.signIn.email({
      email,
      password,
    });
    ...
    if (data) {
      ...
      if (selectedRole === "Writer") {
        ...
        router.push("/dashboard/user"); // Reader দের dashboard/user এ পাঠানো ভালো
      }
      router.refresh();
    }
  ...
  const handleGoogleSignin = async () => {
    try {
      await authClient.signIn.social({
        provider: "google",
        callbackURL: "/dashboard/user", // গুগল লগইনের পর ইউজার ড্যাশবোর্ডে যাবে
      });
    } catch (err) {
      toast.error("Google sign-in failed");
    }
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gray-50 px-4">
      <div className="bg-white p-8 rounded-[2rem] border border-gray-200 w-[350px] shadow-xl">
        <div className="text-center mb-6">
          <h1 className="text-xl font-bold text-gray-900">Welcome back</h1>
          <p className="text-gray-400 text-xs mt-1">Sign in as {selectedRole}</p>
        </div>

        {/* Role Selection */}
        <div className="flex bg-gray-100 p-1 rounded-xl gap-1 w-full mb-6">
          {roles.map((role) => (
            <button
              key={role}
              type="button"
              ...
              className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg transition-all ${
                selectedRole === role ? "bg-white text-indigo-600 shadow-sm" : "text-gray-500"
              }`}
            >
              {role}
            </button>
        ...
        <Form onSubmit={onSubmit} className="flex flex-col gap-4">
          <TextField isRequired name="email" className="w-full">
            ...
            <Input name="email" type="email" placeholder="you@example.com" className="w-full mt-1 p-2 border rounded-xl" />
          </TextField>

          <TextField isRequired name="password" className="w-full">
            <Label className="text-xs font-bold text-gray-700">Password</Label>
            <div className="relative">
                <Input name="password" type={showPassword ? "text" : "password"} placeholder="••••••••" className="w-full mt-1 p-2 border rounded-xl" />
        ...
        {selectedRole !== "Admin" && (
            <Button onClick={handleGoogleSignin} className="w-full mt-4 bg-white border border-gray-200 py-3 rounded-xl flex items-center justify-center gap-2 font-bold text-xs">
