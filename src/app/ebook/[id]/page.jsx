...
export default function EbookDetailsPage() {
  ...
  // Fetch ebook data using process.env
  useEffect(() => {
    setLoading(true);
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/books/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setEbook(data._id ? data : null);
        setLoading(false);
      })
      .catch(() => {
        setEbook(null);
  ...
  // Check if ebook is bookmarked
  useEffect(() => {
    if (session?.user?.email && ebook?._id) {
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/bookmarks?email=${session.user.email}`)
        .then((res) => res.json())
        .then((data) => {
          const already = data.some((item) => item.ebookId === ebook._id);
  ...
  const handleBookmark = async () => {
    ...
    try {
      if (isBookmarked) {
        await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/bookmarks?email=${email}&ebookId=${ebook._id}`,
          { method: "DELETE" }
        );
  ...
  if (!ebook) return <div className="text-center py-20 text-white">Ebook not found.</div>;
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0e] p-8 text-white">
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-10">
        {/* Cover Image */}
        <div className="w-full md:w-1/3">
          <img src={ebook.coverImage} alt={ebook.title} className="w-full rounded-xl shadow-2xl border border-zinc-700" />
        </div>

        {/* Details */}
        <div className="w-full md:w-2/3 flex flex-col gap-4">
          <h1 className="text-4xl font-bold">{ebook.title}</h1>
          <p className="text-lg text-zinc-400">By {ebook.writerName}</p>
          
          <p className="text-2xl font-semibold text-emerald-400">${ebook.price}</p>
          <p className="text-zinc-300 leading-relaxed">{ebook.description}</p>

          <div className="flex gap-4 mt-4">
            {/* Stripe Form */}
            <form action="/api/checkout_sessions" method="POST">
              <input type="hidden" name="bookId" value={ebook._id} />
              <button
                type="submit"
            ...
            {/* <button
              ...
              className={`px-4 py-2 rounded-lg border border-zinc-600 transition-all ${isBookmarked ? "bg-zinc-700 text-blue-400" : "hover:bg-zinc-800"}`}
            >
